  ...
  return (
    <Dialog open={state.cond} onClose={onClose}>
      <DialogContent className='flex flex-col gap-2 justify-center items-center'>
        <div className={classNames('bg-red-50 w-16 h-16 rounded-4xl flex items-center justify-center')}>
        ...
        <div className='mt-4 flex gap-2'>
          <Button variant='tonal' color='secondary' startIcon={<i className='tabler-x'></i>} onClick={onClose}>
            Cancel
          </Button>
          <Button
            variant='contained'
            color='error'
            startIcon={<i className='tabler-trash'></i>}
            onClick={() => onDelete(state.id, onClose)}
          >
