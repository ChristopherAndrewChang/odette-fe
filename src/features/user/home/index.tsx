  ...
  return (
    <UserContainer>
      ...
      <section className='flex flex-col gap-2 mb-8'>
        ...
        <p className='text-5xl text-white font-bold font-poppins mb-4'>
          Mr/Ms. {localStorage.getItem(STORAGE_KEY.USER_NAME)}
        </p>

        {/* table badge */}
        <div className='px-4 py-1 rounded-full border border-yellow-600/50 w-fit bg-[#281F27] shadow-[0_0_15px_rgba(202,138,4,0.15)]'>
          <p className='text-yellow-500 font-medium font-poppins'>
            Table {localStorage.getItem(STORAGE_KEY.USER_TABLE)}
          </p>
        </div>
      </section>
        ...
        <section className='grid grid-cols-2 mt-16 gap-4'>
          ...
          {data?.data?.menu_enabled ? (
            <CardMenu
              href='/user/menus'
              icon={
                <LuUtensilsCrossed className='text-5xl text-purple-300 mb-6 drop-shadow-[0_0_12px_rgba(216,180,254,0.4)]' />
              }
          ...
          {data?.data?.song_request_enabled ? (
            <CardMenu
              ...
              icon={
                <PiMusicNotesPlus className='text-5xl text-red-300 mb-6 drop-shadow-[0_0_12px_rgba(252,165,165,0.4)]' />
              }
              textHeading='Request'
              textSubtitle='A Song'
