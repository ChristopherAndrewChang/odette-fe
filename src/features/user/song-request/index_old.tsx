...
function SongRequest() {
  ...
  const [openRequest, setOpenRequest] = useState<{ cond: boolean; data: TRequest | null }>({
    cond: false,
    data: null
  })
  ...
  const getDonationsList = (useTemporary?: boolean, multiple?: number) => {
    ...
    let arrAdder = !!multiple
      ? getAdderMultiple(multiple)
  ...
  const { isPending } = useSongRequestMutation({
    onSuccess: () => {
    ...
    <>
      ...
      <PaymentDialog
        ...
        renderButton={
          ...
          >
            {isPending ? <CircularProgress size={20} className='text-white' /> : 'Proceed'}
          </Button>
        }
      />
      <UserContainer>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Controller
              ...
              render={({ field: { value, onChange }, fieldState: { error } }) => (
                <>
                  ...
                  />
                  {!!error?.message ? <p className='mt-2 text-red-500'>This field is required</p> : null}
                </>
              )}
            />
          </div>
          ...
          <Button
            ...
            className='text-white my-3 bg-purple-950'
          >
            {isPending ? <CircularProgress size={20} className='text-white' /> : 'Go to Payment'}
          </Button>

          <p className='text-gray-400 text-sm text-center'>
            Min tip Rp{donations?.data?.song_request?.toLocaleString() || '0'} - Subject to approval
          </p>
        </form>
