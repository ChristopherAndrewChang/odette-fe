...
import { Button, Chip, Skeleton, TextField, Tooltip, useColorScheme } from '@mui/material'
    ...
    <div className='w-full overflow-x-auto'>
      ...
      <div className='flex gap-4 mt-3'>
        <span className={bodyClass(darkMode, 'flex items-center gap-1.5 text-xs')}>
          <span className='w-2.5 h-2.5 rounded-sm inline-block' style={{ background: '#c9a84c' }} />
          Lagu
        </span>
        <span className={bodyClass(darkMode, 'flex items-center gap-1.5 text-xs')}>
          <span className='w-2.5 h-2.5 rounded-sm inline-block' style={{ background: '#2357a8' }} />
          Layar
        </span>
      </div>
    </div>
...
const STATUS_CONFIG = [
  ...
  { key: 'cancelled' as const, label: 'Dicancel', color: '#b83232' }
]
...
export default function ReportPage() {
  ...
  const maxTypeCount = data ? Math.max(...Object.values(data.screen_types).map(t => t.count), 1) : 1
